...
test_that("Standardization throws errors for malformed data", {
  ...
  Znew[1] <- NA
  expect_error(
    standardize(X, target, Znew, lambda = 0, verbose = F),
  ...
  Xnew[1, 1] <- NA
  expect_error(
    standardize(Xnew, target, Z, lambda = 0, verbose = F),
  ...
  targetnew[1] <- NA
  expect_error(
    standardize(X, targetnew, Z, lambda = 0, verbose = F),
