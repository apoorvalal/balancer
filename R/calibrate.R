...
calibrate <- function(formula, target_count, data,
  ...
  # get weights
  weights <- calibrate_(
    cells %>% select(-sample_count, -target_count),
...
calibrate_ <- function(cells, sample_counts, target_counts, order = NULL,
  ...
  constraints <- create_rake_constraints(
    cells, D, sample_counts,
  ...
  qvec <- create_rake_qvec(
    D, sample_counts, target_counts, lambda,
    prob_weights
  )
