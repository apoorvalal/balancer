...
standardize_rct <- function(X, trt, target, Z,
  ...
  if (is.null(data_in$constraints)) {
    constraints <- create_constraints_rct(
      Xz, trt, target, Z, lowlim,
      uplim, exact_global,
  ...
  if (verbose) {
    settings <- osqp::osqpSettings(
      verbose = TRUE, eps_abs = 1e-6,
      eps_rel = 1e-6, max_iter = 5000
    )
  } else {
    settings <- osqp::osqpSettings(
      verbose = FALSE, eps_abs = 1e-6,
      eps_rel = 1e-6, max_iter = 5000
    )
  }
...
create_constraints_rct <- function(Xz, trt, target, Z,
  ...
  consts <- create_constraints(
    Xz, target, Z, lowlim, uplim,
...
check_data_rct <- function(X, trt, target, Z, Xz, lambda,
  ...
  if (length(trt) != nrow(X)) {
    stop(
      "The number of rows in covariate matrix (", n,
