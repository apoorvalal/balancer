...
cluster_weights <- function(ind_covs, clus_covs, trt, clusters,
  ...
  constraints <- create_constraints_cluster(
    ind_covs, clus_covs, trt, clusters[trt == 0],
...
create_q_vector_cluster <- function(ind_covs, clus_covs, trt, m) {
  ...
  q <- -c(
    colMeans(ind_covs[trt == 1, , drop = F]),
...
compute_cluster_se <- function(y, wts, trt, clusters, m1hat, m0hat) {
  ...
  return(data.frame(
    Estimand = c("Average Outcome for Treated", "Average Counterfactual Outcome for Treated", "ATT"),
